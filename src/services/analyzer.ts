...
const systemPrompt = `你是台灣廣告法規風險判讀專家，專門協助 KOL、品牌、商家與消費者檢查廣告內容是否涉及違規、誇大或誤導。
...
補充原則：
- 若內容包含療效宣稱、保證效果、絕對性用語或其他高風險特徵，請明確指出不可直接作為購買或發布依據
- revisedContent 應改寫成保守、中性的資訊描述，不要保留原本的誇大承諾
- 不要直接代替主管機關認定違法，應使用「可能涉及」「高風險特徵」等謹慎措辭

風險等級定義：
- safe (0-20分)：未見明顯違規風險，但仍建議最後人工確認
...
function getAudienceContext(audienceMode: ReviewAudienceMode): string {
    if (audienceMode === 'consumer') {
        return `
【使用情境：消費者風險辨識】
請用「協助消費者判斷廣告是否可疑」的角度回答：
1. issues 請指出廣告中的可疑重點與可能涉及的法規風險
2. suggestions 請提供消費者可採取的查證建議，例如保存截圖、檢查標示、查詢法規或主管機關資訊
3. revisedContent 請提供一個「較安全的表述示例」，幫助使用者理解原文為何可疑
4. 請避免直接下結論說對方一定違法，應使用「可能涉及」「高風險特徵」這類措辭
`
    }

    return `
【使用情境：經營者送審】
請用「協助經營者在發布前降風險」的角度回答：
1. issues 請指出需要修改的高風險說法
2. suggestions 請提供可直接採用的改稿建議
3. revisedContent 請提供一個較安全、可作為修改方向參考的版本
`
}
...
function buildUserPrompt(
    ...
    return `
請分析以下 ${getProductTypeName(productType)} 的廣告、商品頁文案或私訊內容：

【使用者需求】
${getAudienceContext(audienceMode)}
...
async function analyzeWithOpenAI(
    ...
    const { text } = await generateText({
        model: openai('gpt-4o'),
        system: systemPrompt,
        prompt: buildUserPrompt(content, productType, audienceMode),
        temperature: 0.3,
    })
...
export function mockAnalyzeContent(
    content: string,
    productType: ProductType,
    audienceMode: ReviewAudienceMode = 'business'
): AnalysisResult {
    const issues: Issue[] = []
    let riskScore = 0
    ...
    return {
        ...
        suggestions: issues.length > 0
            ? audienceMode === 'consumer'
                ? [
                    '先保留廣告截圖與商品頁資訊，避免之後內容被刪改。',
                    '檢查產品標示、主管機關核准資訊與是否有誇大療效宣稱。',
                    '若涉及健康、醫療或保證效果說法，建議再查詢法規或向主管機關反映。',
                ]
                : [
                    '建議將療效相關用語改為較中性的表述。',
                    '避免使用絕對性或保證性的詞彙。',
...
export async function analyzeContent(
    ...
    try {
        switch (provider) {
            case 'openai':
                return await analyzeWithOpenAI(content, productType, audienceMode)
            case 'anthropic':
                return await analyzeWithAnthropic(content, productType, audienceMode)
            case 'gemini':
                return await analyzeWithGemini(content, productType, audienceMode)
            case 'mock':
            default:
                return mockAnalyzeContent(content, productType, audienceMode)
        }
    } catch (error) {
